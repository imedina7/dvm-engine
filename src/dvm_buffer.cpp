 ...
 */
VkDeviceSize DvmBuffer::getAlignment(VkDeviceSize instanceSize,
...
DvmBuffer::DvmBuffer(DvmDevice& device,
...
{
  unmap();
  vkDestroyBuffer(dvmDevice.device(), buffer, nullptr);
  vkFreeMemory(dvmDevice.device(), memory, nullptr);
}
 ...
 * @return VkResult of the buffer mapping call
 */
VkResult DvmBuffer::map(VkDeviceSize size, VkDeviceSize offset)
{
  assert(buffer && memory && "Called map on buffer before create");
  return vkMapMemory(dvmDevice.device(), memory, offset, size, 0, &mapped);
}
 ...
 */
void DvmBuffer::unmap()
{
  if (mapped) {
    vkUnmapMemory(dvmDevice.device(), memory);
    mapped = nullptr;
  }
}
 ...
 *
 */
void DvmBuffer::writeToBuffer(void* data,
                              VkDeviceSize size,
                              VkDeviceSize offset)
{
 ...
 */
VkResult DvmBuffer::flush(VkDeviceSize size, VkDeviceSize offset)
{
  ...
  return vkFlushMappedMemoryRanges(dvmDevice.device(), 1, &mappedRange);
}
 ...
 */
VkResult DvmBuffer::invalidate(VkDeviceSize size, VkDeviceSize offset)
{
  VkMappedMemoryRange mappedRange = {};
  mappedRange.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
  mappedRange.memory = memory;
  mappedRange.offset = offset;
  mappedRange.size = size;
  return vkInvalidateMappedMemoryRanges(dvmDevice.device(), 1, &mappedRange);
}
 ...
 */
VkDescriptorBufferInfo DvmBuffer::descriptorInfo(VkDeviceSize size,
                                                 VkDeviceSize offset)
{
 ...
 *
 */
void DvmBuffer::writeToIndex(void* data, int index)
{
 ...
 *
 */
VkResult DvmBuffer::flushIndex(int index)
{
 ...
 */
VkDescriptorBufferInfo DvmBuffer::descriptorInfoForIndex(int index)
{
 ...
 * @return VkResult of the invalidate call
 */
VkResult DvmBuffer::invalidateIndex(int index)
{
