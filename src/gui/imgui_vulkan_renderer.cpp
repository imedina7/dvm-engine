...
//  ImGuiPlatformIO interface. 2023-01-02: Vulkan: Fixed sampler passed to
//  ImGui_ImplVulkan_AddTexture() not being honored + removed a bunch of
...
#include "imgui_vulkan_renderer.h"
#include <stdio.h>
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer)
{
  ...
  // Create the Upload Buffer:
  {
    ...
    buffer_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    err = vkCreateBuffer(
        v->Device, &buffer_info, v->Allocator, &bd->UploadBuffer);
    check_vk_result(err);
    VkMemoryRequirements req;
    ...
    err = vkAllocateMemory(
        v->Device, &alloc_info, v->Allocator, &bd->UploadBufferMemory);
    check_vk_result(err);
    err = vkBindBufferMemory(
        v->Device, bd->UploadBuffer, bd->UploadBufferMemory, 0);
    check_vk_result(err);
  }
  ...
  // Upload to Buffer:
  {
    char* map = nullptr;
    err = vkMapMemory(
        v->Device, bd->UploadBufferMemory, 0, upload_size, 0, (void**)(&map));
    check_vk_result(err);
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
  ...
  ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
  VkResult err =
      (VkResult)platform_io.Platform_CreateVkSurface(viewport,
                                                     (ImU64)v->Instance,
  ...
  vkGetPhysicalDeviceSurfaceSupportKHR(
      v->PhysicalDevice, v->QueueFamily, wd->Surface, &res);
  if (res != VK_TRUE) {
  ...
  wd->PresentMode =
      ImGui_ImplVulkanH_SelectPresentMode(v->PhysicalDevice,
                                          wd->Surface,
                                          &present_modes[0],
          ...
          (ImGui_ImplVulkan_ViewportData*)viewport->RendererUserData)
  {
    ...
    if (vd->WindowOwned)
      ImGui_ImplVulkanH_DestroyWindow(
          v->Instance, v->Device, &vd->Window, v->Allocator);
    ImGui_ImplVulkanH_DestroyWindowRenderBuffers(
...
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport* viewport, void*)
{
  ...
  ImGui_ImplVulkan_RenderDrawData(
      viewport->DrawData, fd->CommandBuffer, wd->Pipeline);
