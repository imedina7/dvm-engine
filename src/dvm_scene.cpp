...

{
    ...
    registry.emplace<TransformComponent>(viewerEntity, glm::vec3(0.f, -1.f, -3.f));

    auto& device = DvmApp::getInstance().getDevice();

    camera.setViewDirection(glm::vec3(0.f, 10.f, 2.5f),
                            glm::vec3(0.f, 0.f, 0.f));

    auto box = registry.create();

    registry.emplace<TransformComponent>(box, glm::vec3(0.f, 0.f, 0.f), glm::vec3(1.f), glm::vec3(0.f, glm::pi<float>(), 0.f));
    registry.emplace<ModelComponent>(
        box,
        DvmModel::createModelFromFile(device, "models/shading_test_box.obj"));

    std::vector<entt::entity> lights {registry.create()};

    for (auto lightEntity : lights) {
      registry.emplace<TransformComponent>(lightEntity, glm::vec3(0.f, 0.f, -2.f), glm::vec3(.1f), glm::vec3(0.f));

      registry.emplace<PointLightComponent>(lightEntity, 0.2f, glm::vec3(1.f, 1.f, 1.f));
    }
  }

  void Scene::update(float frameTime, glm::vec2 mouseDelta) {
    cameraController.moveInPlaneXZ(camera, frameTime, registry, viewerEntity, mouseDelta, 0.1f);
